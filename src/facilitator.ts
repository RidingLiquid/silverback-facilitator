...
export async function initializeSvm(privateKeyInput: string): Promise<void> {
  try {
    ...
    const svmSigner = {
      ...
      // Override sendTransaction to skip preflight — the SDK already simulates
      // explicitly in verify(), so the RPC's redundant preflight check just
      // adds latency and causes stale-blockhash failures (-32002).
      sendTransaction: async (transaction: string, network: string) => {
        console.log(`[SVM] sendTransaction: network=${network}, txLen=${transaction.length}`);
        try {
          const { createRpcClient: createRpc, normalizeNetwork } = await import('@x402/svm');
          const rpc = createRpc(normalizeNetwork(network));
          const sig = await rpc.sendTransaction(transaction, {
            encoding: 'base64' as any,
            skipPreflight: true,
            maxRetries: 3n as any,
          }).send();
          console.log(`[SVM] sendTransaction SUCCESS: ${sig}`);
          return sig;
        } catch (err: any) {
          console.error(`[SVM] ❌ sendTransaction FAILED:`, err.message || err);
          throw err;
        }
      },
    };
